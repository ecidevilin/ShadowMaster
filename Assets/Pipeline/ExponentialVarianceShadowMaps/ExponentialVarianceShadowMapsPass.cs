...
namespace UnityEngine.Experimental.Rendering.LightweightPipeline.Extension
{
    public class ExponentialVarianceShadowMapsPass : ScriptableRenderPass
    {
        ...
        const string _KernelFirstFilteringName = "KernelFiltering9Taps";
        ...
        public override void Execute(ScriptableRenderer renderer, ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            CommandBuffer cmd = CommandBufferPool.Get(_KernelFirstFilteringName);
            ...
            using (new ProfilingSample(cmd, _KernelFirstFilteringName))
            {
                ...
                int tgx = (int)((w + _NumThreadX - 3) / (_NumThreadX - 2));
                int tgy = (int)((w + _NumThreadY - 3) / (_NumThreadY - 2));

                //cmd.SetComputeIntParam(_Compute, _Compute_Vertical, 0);
                cmd.SetComputeTextureParam(_Compute, _KernelFirstFiltering, _Compute_OutputTex, drti);
                cmd.DispatchCompute(_Compute, _KernelFirstFiltering, tgx, tgy, 1);
